...
{if $view_mode != "list" && $view_mode != "single_button"}
    ...
    {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="departments.picker?display=`$display`&extra=`$extra_var`&department_director=`$department_director`&picker_for=`$picker_for`&data_id=`$data_id`&shared_force=`$shared_force``$extra_url`"|fn_url but_role="add" but_target_id="content_`$data_id`" but_meta="cm-dialog-opener `$but_meta`" but_icon=$but_icon}
...
{if $view_mode == "single_button"}
    ...
    {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="departments.picker?display=`$display`&department_director=`$department_director`&picker_for=`$picker_for`&extra=`$extra_var`&checkbox_name=`$checkbox_name`&root=`$default_name`&except_id=`$except_id`&data_id=`$data_id``$extra_url`"|fn_url but_role="text" but_icon="icon-plus" but_target_id="content_`$data_id`" but_meta="`$but_meta` cm-dialog-opener add-on btn"}
